...
class SubtitleApp:
    def __init__(self, root):
        self.root = root
        self.root.title("AI Subtitle Master Pro")
        ...
        self.log_box = tk.Text(root, height=10, width=70, bg="#1a1a1a", fg="#2ecc71", font=("Consolas", 9))
    ...
    def start_logic(self):
        try:
            ...
            for i in range(0, len(blocks), c_size):
                ...
                prompt = f"Translate the following SRT subtitles to natural {target}. Keep original SRT formatting:\n\n{batch}"
                response = model.generate_content(prompt)
                if response.text:
                    clean = response.text.replace('```srt', '').replace('```', '').strip()
                    output_content += clean + "\n\n"
                self.log(f"Completed chunk { (i//c_size) + 1 }")
                time.sleep(10) # 429 Error Safety Delay
